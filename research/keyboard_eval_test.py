...
thres_params = ThresholdParams(block_scale=40, sub_threshold=-30, retr_type=1L, retr_approx=2L, min_poly=5)
edge_detector = ContoursExtractor(EdgeDetector(edge_params), extract_params)
thres_detector = ContoursExtractor(ThresholdDetector(thres_params), extract_params)
LetterExtractionMetric.test_layouts(layouts, ChunkExtractor(45))
